...
class UnbinnedLH_ass:  # override the class with a better one
    
    def __init__(self, f, data, dataY, sig_l, sig_u, weights=None, bound=None, badvalue=-100000, extended=False, extended_bound=None, extended_nint=100):
        ...
        self.sig_l = np.array(sig_l)
        self.sig_u = np.array(sig_u)
    ...
    def __call__(self, *par):  # par are a variable number of model parameters
        ...
        # compute the function value
        f = compute_f_ass(self.f, self.data, *par)
        
        sig1 = 2*self.sig_l*self.sig_u/(self.sig_l+self.sig_u)
        sig2 = (self.sig_u-self.sig_l)/(self.sig_l+self.sig_u)
        
        logL = -1/2*(self.dataY-f)**2/(sig1*sig2(self.dataY-f)**2
